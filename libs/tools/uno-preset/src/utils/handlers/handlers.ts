...
export function px(str: string) {
  if (unitOnlyRE.test(str)) return `1${str}`;
  const match = str.match(numberWithUnitRE);
  if (!match) return;
  const [, n, unit] = match;
  const num = Number.parseFloat(n);
      ...
      .replace(/(?:calc|clamp|max|min)\((.*)/g, match => {
        ...
        return match
          .replace(/var\((--.+?)[,)]/g, (match, g1) => {
            vars.push(g1);
            return match.replace(g1, '--bh-calc');
          })
          .replace(
            /(-?\d*\.?\d(?!\b-\d.+[,)](?![^+\-/*])\D)(?:%|[a-z]+)?|\))([+\-/*])/g,
            '$1 $2 ',
          )
          .replace(/--bh-calc/g, () => vars.shift()!);
      });
  }
}
...
export function time(str: string) {
  const match = /^(-?[0-9.]+)(s|ms)?$/i.exec(str);
  if (!match) return;
  const [, n, unit] = match;
  const num = Number.parseFloat(n);
...
export function degree(str: string) {
  const match = /^(-?[0-9.]+)(deg|rad|grad|turn)?$/i.exec(str);
  if (!match) return;
  const [, n, unit] = match;
  const num = Number.parseFloat(n);
