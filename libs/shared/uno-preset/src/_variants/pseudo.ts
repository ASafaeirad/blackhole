...
const PseudoClassesColon: Record<string, string> = Object.fromEntries(
  [['backdrop', '::backdrop']].map(key =>
    Array.isArray(key) ? key : [key, `:${key}`],
  ),
);
...
function taggedPseudoClassMatcher(
  ...
  const matchBracket = (
    input: string,
  ): [label: string, rest: string, prefix: string] | undefined => {
    ...
    return [
      ...
      bracketValue.includes('&')
        ? bracketValue.replace(/&/g, prefix)
        : `${prefix}${bracketValue}`,
    ];
  };
  ...
  const matchPseudo = (
    ...
    | [label: string, rest: string, prefix: string, pseudoKey: string]
    | undefined => {
    ...
    const label = match[3] ?? '';
    let pseudo =
      PseudoClasses[pseudoKey] ||
      PseudoClassesColon[pseudoKey] ||
      `:${pseudoKey}`;
  ...
  return {
    ...
    match(input, ctx) {
      ...
      const result =
        matchBracket(input) ?? matchPseudo(input) ?? matchPseudoVar(input);
      ...
      return {
        ...
        handle: (input, next) =>
          next({
            ...input,
            prefix: `${prefix}${combinator}${input.prefix}`.replace(
              rawRE,
              '$1$2:',
            ),
            sort:
              PseudoClassesKeys.indexOf(pseudoName) ??
              PseudoClassesColonKeys.indexOf(pseudoName),
          }),
      };
    },
    multiPass: true,
...
export function variantPseudoClassesAndElements(): VariantObject {
  ...
  return {
    ...
    match(input, ctx) {
      ...
      const match =
        input.match(PseudoClassesAndElementsRE) ??
        input.match(PseudoClassesAndElementsColonRE);

      if (match) {
        const pseudo =
          PseudoClasses[match[1]] ||
          PseudoClassesColon[match[1]] ||
          `:${match[1]}`;
...
export function variantPseudoClassFunctions(): VariantObject {
  ...
  return {
    match(input, ctx) {
      ...
      const match =
        input.match(PseudoClassFunctionsRE) ??
        input.match(PseudoClassColonFunctionsRE) ??
        input.match(PseudoClassVarFunctionRE);
      ...
      if (match) {
        ...
        const pseudo = fnVal
          ? h.bracket(match[2])
          : PseudoClasses[match[2]] ||
            PseudoClassesColon[match[2]] ||
            `:${match[2]}`;
        return {
          matcher: input.slice(match[0].length),
