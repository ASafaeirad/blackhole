...
it('pseudo variant order', async () => {
  const uno = createGenerator({
    variants: [variantPseudoClassesAndElements()],
    rules: [[/^foo-(\d)$/, ([_, a]) => ({ text: `foo-${a}` })]],
  });

  const css = await uno
    .generate(['foo-1', 'hover:foo-2', 'focus:foo-3', 'disabled:foo-4'])
    .then(r => r.css);
  ...
  const css2 = await uno
    .generate(['foo-1', 'hover:foo-1', 'focus:foo-1', 'disabled:foo-1'])
    .then(r => r.css);
...
it('focus-visible:', async () => {
  ...
  const result = await uno.generate(['focus-visible:foo-1', 'focus:foo-2']);

  expect(result.matched).toMatchInlineSnapshot(`
      Set {
        "focus-visible:foo-1",
        "focus:foo-2",
      }
