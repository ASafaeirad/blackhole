...
function rowColTheme(s: string) {
  return s.startsWith('r') ? 'Row' : 'Column';
}
...
export const grids: Rule<Theme>[] = [
  ...
  [
    ...
    ([, c, v], { theme }) => ({
      [`grid-${rowCol(c)}`]:
        theme[`grid${rowColTheme(c)}`]?.[v] ?? h.bracket.cssvar.auto(v),
    }),
  ],
  ...
  [
    /^(?:grid-)?(row|col)-start-(.+)$/,
    ([, c, v]) => ({ [`grid-${rowCol(c)}-start`]: h.bracket.cssvar(v) ?? v }),
  ],
  [
    /^(?:grid-)?(row|col)-end-(.+)$/,
  ...
  [
    /^(?:grid-)?auto-(rows|cols)-(.+)$/,
    ([, c, v], { theme }) => ({
      [`grid-auto-${rowCol(c)}`]: autoDirection(c, theme, v),
    }),
  ...
  [
    ...
    ([, v]) => ({ 'grid-auto-flow': h.bracket.cssvar(v) }),
  ],
  [
  ...
  [
    ...
    ([, c, v], { theme }) => ({
      [`grid-template-${rowCol(c)}`]:
        theme[`gridTemplate${rowColTheme(c)}`]?.[v] ?? h.bracket.cssvar(v),
    }),
  ],
  [
